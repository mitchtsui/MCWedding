...
- **Floor-plan SVG matches the Peninsula's real Salisbury Room / Foyer plan (2026-08-17).** Geometry traced from the hotel's own floor plan (`SynologyDrive/MC Wedding/034ec716….png`, 0.82 scale, same orientation): **15 tables** — Room holds 11·9 (flanking a centred 12ft × 8ft stage with the mock cake beside it), 10·6·3·7, 8·2·1·5; Foyer holds 12·13, 15, 16·17. **Tables 4 and 14 are deliberately skipped** (numbering superstition — this is correct, don't "fix" it). Built-in screens, mirror TVs, built-in TVs, pillars and the reception furniture are drawn as wall fixtures. The invented "LIVE BAND" box from the old layout is gone (it isn't on the real plan). Verified programmatically: all 15 seat rings clear each other (≥103px centre spacing), every ring inside its room's walls, no ring touches the stage/TVs/furniture, positions within 0.6px of the scaled source. Style classes unchanged (`fp-frame`/`fp-stage`/`fp-table`/`fp-seat-dots` + new `fp-wall`/`fp-fixture`/`fp-label`/`fp-area-label`). **Tables 16/17 are new to the site** — the lock/unlock and occupant JS is generic (`data-table` driven), nothing else needed updating.
...
- **2026-08-17: rebuilt as the invitation itself.** Per user request: *"there's wedding-day-garden image with similar info again, and totally different typo style. I hate this… Just because our wedding card is in that style, doesn't mean we need to put an 'image' on it. Think of ways to incorporate elements from our wedding card."*
  ...
  **What it is now:** an `.invite-card` — the printed invitation rebuilt in HTML/CSS/inline SVG from the card's own vocabulary. Ivory panel (`#FBF6EC`) on sage (`#DCE3D5`), gold double rule with four mirrored corner flourishes, the MC monogram in a gold oval, the formal wording ("Together with their families" / script names / "request the pleasure of your company"), date, venue, address, attire, and the 囍 seal in the card's red. Below it, the timeline is rendered in the card's **line-art icon** style (temple / rings / champagne in gold circles) instead of the old bare `.timeline-*` grid. This is now the **single** place venue/date/attire are stated on the site.
  ...
  - The garden artwork is **kept but demoted** to a decorative band at the foot of the section with nothing set on top of it, served as **WebP (139 KB, down from 2135 KB — a 93.5% saving)** via `<picture>` with a 207 KB JPEG fallback. The original PNG stays in `photos/` as the source asset (same convention as `photos/gallery/`) but is no longer served.
...
- Timeline content (17:00–18:00, 18:30–19:30, 19:45–23:00) and the couple's copy are **unchanged** — only the presentation moved.
  - ℹ️ The printed card reads "Dinner 8:00 pm" where the site says 19:45. Confirmed with the couple 2026-08-17: **both are fine**, 19:45 is the real call time and 8:00pm is the card's rounding. Left as is deliberately — do not "fix" this.
...
- **The Wedding Day section is an HTML/CSS/SVG rebuild of the printed invitation** (`.invite-card` + icon timeline), not an image with text on it. The garden artwork is a decorative band at the foot of the section, served as `photos/wedding-day-garden.webp` (139 KB) with a `.jpg` fallback via `<picture>`. The 2135 KB `.png` is kept only as the source asset and must not be served. Never set text over that artwork again — that is the exact thing the rebuild removed.
