...
- `README.md` — setup, Vercel + Supabase wiring, RSVP and seating flow, guest-list workflow, WhatsApp tool. Stale as of 2026-09-22, fix in the same PR: "15 tables", "198 guests" and the May stats block; the file table (missing `admin.html`, `preview.html`, `api/`, `vercel.json`, the audit SQL, and the 2026-09-22 rundown — `wedding-day.html`, its three JS files, `tests/`, `docs/`); `seed-guests.js` advertised as an alternative seed; "all assets base64 / drag-and-drop deploy"; "see their seat" / "View Floor Plan" (table only, "View Seating Plan"); the "Status" dropdown (now "Outreach", plus the Attending control); the section order; the go-live checklist; the Redirect URLs list (outreach needs its own).
