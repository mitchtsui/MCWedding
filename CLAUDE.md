...
- **2026-09-03 (later same day): Table 9 moved into that vacated left slot, per user request.** Now flanks the stage at `translate(189, 111)` — the exact spot table 11 held before it moved to the Foyer — joining 10/6/8/2 as the Room's left cluster (5 tables). This closes the gap the previous change left; nothing flanks the stage on the right any more, by request ("leave some empty space in the right upper area"). The right cluster (1, 3, 5, 7) was redrawn as an even 2×2 to fill the space freed by 9's move: `translate(540, 214)` / `translate(672, 214)` top row (3, 7), `translate(540, 308)` / `translate(672, 308)` bottom row (1, 5) — 132px between columns, 94px between rows (the same row spacing already used between tables 6 and 2 on the left, reused here for consistency rather than picked arbitrarily). Table 1 stays the left-hand one of the pair closest to the structural column/TV, matching the existing "TV hangs on the column behind table 1" comment. Verified via Playwright: no table overlaps another, the bottom row clears the structural column and its TV strip (which stayed in place as real venue fixtures, per the tables-16/17 removal precedent above), and the upper-right area beside the stage is now visibly empty.
