...
- **`vw`/`vh` units elsewhere were deliberately left alone** (e.g. the hero's `clamp()` font-sizes and `--hero-photo-h`, from the previous desktop-rebalance change). These still measure the true browser viewport, not the frame — on a wide desktop viewport they just resolve to their `clamp()` upper bound instead of scaling further, and since those bounds were already tuned to be mobile-appropriate (including large-phone sizes), the result still looks right inside the ~430px frame. Converting these to CSS container query units (`cqw`/`cqh`) for pixel-perfect frame-relative scaling was considered and rejected as unnecessary complexity/risk for a difference that isn't visually meaningful in practice.
...
Verified via Playwright at 1024×768, 1440×900, and 1920×1080 (desktop-as-frame) and 390×844 (real mobile, confirmed pixel-identical to before this change): frame renders centred at the correct width with plain gardenia on either side; bottom nav bar stays genuinely fixed to the visible viewport while scrolling (checked at multiple scroll positions, not just page load); the lightbox and floor-plan modal are width-capped to the frame; the Chambolle egg pops up hugging the frame's edge (not the true viewport's edge) with its speech bubble correctly placed; the "More" accordion opens/closes correctly inside the frame; no horizontal overflow at any tested width; no JS console errors.
...
- **The whole site renders as a mobile view at every browser width** (2026-08-13, see "Site Frame" at the top of section 4) — `#site-frame` caps the page to `var(--frame-max)` (430px), and all the old `@media` breakpoints are `@container` queries keyed to that frame now, not the real viewport. Any new `@media (max-width: ...)` you add won't participate in this system — use `@container` to match the existing pattern. Any new `position: fixed` element needs manual width-capping/centering (`left:50%;transform:translateX(-50%);width:min(var(--frame-max),100vw)`, or `--frame-gap` for edge-offset elements) — it will not automatically respect the frame.
