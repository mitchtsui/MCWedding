...
- **Mobile (≤700px), `.bottom-nav`:** 6 tabs — Home · Day · Travel · Q&A · RSVP · Photos. Travel and Q&A got their own tabs; the old 5th "More" tab (which used a **clock** icon for a section containing none of those things) is gone. Each icon now depicts its destination.
  - **2026-08-18: restyled "frost-in-place"** — same full-width, flush-to-bottom geometry as before (so body padding and Chambolle-egg clearances are untouched), but the hard `border-top: 1px solid var(--divider)` and opaque background are replaced with frosted glass (`backdrop-filter: blur(16px) saturate(1.5)` behind `@supports`, 94%-opaque solid fallback), a light inner hairline, and a soft upward shadow. Active tab's icon circle widens 34px → 52px into a golden stadium (transition covered by the global reduced-motion block). An alternative "floating pill" treatment (detached, rounded, different clearances) exists on the `claude/bottom-nav-pill` branch — the two differ only in bar geometry.
