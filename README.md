...
- **Horizontal day timeline** (scrolls sideways inside itself; the page itself scrolls vertically), rows The day → Bride → Bridesmaids → Groom → Groomsmen, with bride/groom activities, bridesmaid/groomsman duties and locations, plus the tea, photo-order and supplies reference lists. The hour ruler stays visible while you scroll down the lanes and the Now/Preview chip has its own strip under the hour labels (v5, 2026-09-23).
- **Phone layout** (540 px and below): the name/team selector sits above the task cards, the two next tasks are compact rows, search is a disclosure with a "Clear filter" control, and the prototype notice, role note and "Preview a time" control fold into "Rundown notes & preview time" below the timeline. The layout block at the top of `wedding-day.js` moves the same nodes into phone slots; nothing is duplicated.
