...
This is the dated build journal that lived in `CLAUDE.md` until 2026-09-22, moved here **unchanged** (heading levels shifted down one step; two dated ⚠️ STALE notes and the "backs CLAUDE.md §…" section headings are the only additions). It is history and measurements, **not rules** — the rules live in `CLAUDE.md`. When this log and `CLAUDE.md` disagree, `CLAUDE.md` wins; when either disagrees with the files, the files win (and the doc gets fixed).

Sections keep the old `CLAUDE.md` order (§3, §4, §4b, §5, §5b, §6, §7, §9); the old §1, §2 and §8 were superseded by the new `CLAUDE.md` and were not carried over. Convention from here on: **newest entries at the top of each section** (the moved text is left in the order it was written).

**To add an entry:** find the section for the area you touched (or add a new `## ` section at the end), put a dated line or paragraph (`**YYYY-MM-DD: what changed, why, and the measurement or verification that backs it**`) at the top of that section, and if it changes a rule, change `CLAUDE.md` in the same commit. Never rewrite an older entry — add a dated note above it (the two ⚠️ STALE notes below are the pattern).

---

## Design System (old CLAUDE.md §3) — backs CLAUDE.md §3: the two base64 `@font-face` blocks, the Besotted Love fixes (`padding-top: 0.48em`, string-specific `clamp()` sizes), the inverted `.hero-script` / `.hero-names` class names, and the AA colour values (`--text-light`, "and Chambolle")

### 3. Design System

#### Colour Palette (CSS Variables)
> ⚠️ **STALE — noted 2026-09-22.** This table no longer matches `wedding-invitation.html`'s `:root`, which is the truth. 5 of the 10 values differ: `--rose-bisque` is `#DDC8D0` in the file (the Pantone value — "and Chambolle" stopped using it and has been `#7A5A66` since 2026-08-17), `--text` is `#2E2E1C`, `--text-mid` is `var(--olive)`, `--text-light` is `#58664A` (its own AA value since 2026-08-17, see "Typography & accessibility floor" below) and `--divider` is `#D8CEBC`. `--gardenia`, `--golden`, `--pale-peach`, `--olive` and `--sea-spray` still match. The `:root` also defines `--bg`, `--bg-card`, `--accent*`, `--white`, the glass tokens and (scoped to `#wedding-day`) `--card-gold`, none of which are listed here. Kept as written, for history.
...
## Website (old CLAUDE.md §4) — backs CLAUDE.md §3 (responsive via `@media` and the `#site-frame` bans, section order and `#more`, hero `padding-top` ↔ `--hero-photo-h`, accordions without `max-height`, glass vs paper, nav clearances and scroll-spy, i18n, table-only post-RSVP, the plain Wedding Day card and its icons, gallery workflow, the 0.7rem type floor), §1 (dinner 20:00 in three places), §4 (`p_plus_one_name` / PGRST202, the dashboard SQL editor) and §6 (verification widths)
...
- Fields: name, email, **contact number / WhatsApp** (added 2026-08-13, `#f-phone`, `type="tel"`), attendance toggle, dietary, **song request** (live band), message. **The plus-one field was removed 2026-08-25** — see below.
...
- **2026-08-25: the plus-one field is gone from the form**, per user request (*"Remove the +1 from the RSVP"*). `#plusone-group`, `#f-plusone` and its label are deleted, `setAttendance()` no longer shows/hides that group, and `submitRSVP()` no longer reads it.
  - ⚠️ **`p_plus_one_name` is STILL SENT in the RPC payload, hardcoded to `null` — do not "tidy" it away.** The live `submit_rsvp` declares `p_plus_one_name TEXT` with **no DEFAULT** (see `supabase/migrations/2026-08-17_submit_rsvp_phone.sql`), so dropping the key makes PostgREST fail to resolve the 9-arg function and **every submit dies with PGRST202** — the exact outage the `p_phone` rollout caused on 2026-08-17. Removing the argument requires a DB migration first, and the frontend must not ship ahead of it.
  - The `rsvp.plus_one_name` column, the `pending_plus_ones` view and the `admin_rsvps` view are **untouched**. Historical rows keep their values; the view simply stops receiving new ones. Guests already on the master list as their own row (e.g. `Flora Ding plus one`) are unaffected — they were never entered through this field, they show up in the member picker.
  - The Q&A answer *"Can I bring a plus-one?"* was rewritten in **all three languages**: it used to promise *"you'll be able to add their name when you RSVP"*, which the form no longer offers. It now says a named plus-one is already on the guest list and will appear when the invitation is opened. The policy itself is unchanged.
  - i18n: `label[for="f-plusone"]` and `#f-plusone::placeholder` removed from both the zh and ja packs (they would otherwise log `[i18n] no match:`).
  - Verified in-browser in all three languages with a demo invite code: field absent, form is Name → Email → Phone → Attending → Dietary → Song → Message, `setAttendance('no'/'yes')` no longer throws, a full demo submit still sends all **9** RPC args and lands on the seat card, and **zero `[i18n] no match:` warnings**.
...
- **2026-09-03 (same day): Table 11 moved from the Room into the Foyer, per user request**, joining 12/13/15 as a 2×2 square (172px between columns, 172px between rows, centred on the foyer's width) — top row 11 (left, `translate(304, 494)`) / 13 (right, `translate(476, 494)`), bottom row 12 (left, `translate(304, 666)`) / 15 (right, `translate(476, 666)`). These row positions were chosen to land on the existing mirror-TV / built-in-TV fixture pair (y 472–542 and y 611–681) rather than arbitrary values, so the square reads as sitting between the two TV pairs rather than off-center against them. The Room dropped to **9 tables** (11 removed from its old `translate(189, 111)` spot flanking the stage) and the Foyer rose to **4** — comments updated accordingly. ⚠️ **Table 9 now flanks the stage alone**, with no table on the opposite side where 11 used to sit — a direct, intentional consequence of relocating only 11, not an oversight. Verified via Playwright: all four foyer tables clear each other and the fixtures with margin, no overlap with the structural columns or the Room/Foyer wall.
- **2026-09-03 (later same day): Table 9 moved into that vacated left slot, per user request.** Now flanks the stage at `translate(189, 111)` — the exact spot table 11 held before it moved to the Foyer — joining 10/6/8/2 as the Room's left cluster (5 tables). This closes the gap the previous change left; nothing flanks the stage on the right any more, by request ("leave some empty space in the right upper area"). The right cluster (1, 3, 5, 7) was first redrawn as an even 2×2, then **changed again the same day per user request ("rebalance right side but no need to be 2x2")** to a staggered layout mirroring the left cluster's own zigzag instead of a rigid grid: `translate(538, 214)` / `translate(682, 214)` for 3/7 (mirrors 10/6's row exactly, same 144px spacing), `translate(464, 308)` / `translate(604, 320)` for 1/5 (mirrors 8/2's row, including its 12px vertical stagger between the pair). Table 1 sits at the mirrored position of table 2 — directly at the stage's own right-edge x-coordinate, the same relationship table 2 has with the stage's left edge — keeping it the one closest to the structural column/TV, matching the existing "TV hangs on the column behind table 1" comment. Verified via Playwright: no table overlaps another, table 1 clears the structural column/TV strip by the same 8px margin table 2 already had on the left (precedented, not a new risk), and the upper-right area beside the stage is still visibly empty.
...
- **2026-08-25: seat numbers are no longer shown anywhere post-RSVP, and the floor-plan button moved up into the row block.** Per user request: *"after RSVP, no need to show seat no., instead move view floor plan button there. Change to view seating plan"*. The card's second note already warns that *"Christy may make some final adjustments to the seating plan"* — a per-guest seat number was a promise the data couldn't keep, so each row is now just **name → `Table {n}`**.
  - `.seat-loc .seat` is **deleted**, and `.seat-loc .table` inherits the prominent treatment it used to have (Cormorant Garamond `1.2rem`, `--olive`) instead of staying a small uppercase Raleway label above a bigger number. Verified single-line, no overflow, down to a 344px Z Fold cover screen with the longest realistic pairing ("Christopher Cheung" / "Table 12").
  - The button is renamed **"View Seating Plan"** (was "View Floor Plan") and lives in a new `.seat-card-plan` block **directly under `#seat-card-rows`**, above the notes — it is the answer to "where exactly am I?", so it belongs with the table information, not in the Calendar/Update actions row it came from. `.seat-card-actions` now holds two buttons, not three. **It also carries the `primary` class** (solid olive fill) — moved off "Add to Calendar" the same day at the couple's request, so the card has exactly one filled button and it is the one that answers the question the card raises. "Add to Calendar" and "Update my RSVP" are both plain outlines now.
  - ⚠️ **Consistency was applied to every other post-RSVP surface in the same change** — leaving any of them would have shown the guest a seat number one tap after the card stopped giving one: the floor-plan modal's occupant list (`.fp-occupant .seat` and `.fp-occupant.you .seat` **deleted**; the list still *sorts* by `o.seat`, which is a stable order, not a display), `ownSeatLine()` (the fallback line shown when the floor-plan fetch fails), and `buildDescription()` (the Google Calendar / .ics body, now `Name: Table 5`). If seat numbers are ever wanted back, all four sites must come back together.
  - `seat_number` is untouched in the DB, the RPC payloads, and the demo fixtures — this is a display change only, and the seating planner still assigns 12 seats per table.
  - i18n: the `seatFmt` key was **removed from both the zh and ja `ui` packs** (nothing references it now). The Japanese button label follows the English rename: `フロアマップを見る` → `座席表を見る`; Traditional Chinese already read `查看座位圖`. No `dom` selectors changed — `button[onclick="openFloorPlan()"]` still matches exactly one element.
  - Verified in-browser in all three languages via the demo code (`?code=MC-DEMO2`, demo mode is automatic with no Supabase env locally) with `renderSeatCard()` fed mixed rows (seated / to-be-assigned / declined): rows show table only, button label correct per language, floor-plan modal opens with a names-only occupant list, calendar description emits `Mark Chan: Table 5`, and **zero `[i18n] no match:` console warnings**.
...
- Timeline content (17:00–18:00, 18:30–19:30, 20:00–23:00 — dinner was 19:45 until 2026-08-24, see below) and the couple's copy are **unchanged** — only the presentation moved.
  - ℹ️ **Dinner is 20:00, changed from 19:45 on 2026-08-24 at the couple's request.** This supersedes the 2026-08-17 note that said the site's 19:45 and the printed card's "Dinner 8:00 pm" were both fine and should be left alone — the site now simply matches the card. The time lives in **three** places and they must move together: the visible `.day-time` in the timeline, the `buildDescription()` schedule block that feeds the Google Calendar / .ics export (its lines are space-aligned, so keep any replacement 5 characters wide), and — in a **different file** — all three `DEFAULT_TEMPLATES` in `whatsapp-outreach.html` (bilingual / friends / formal). The outreach templates were missed on 2026-08-24 and still read `7:45pm` until 2026-08-25; this note said "two" places, which is why. The formal template's schedule block is space-aligned too. Not affected, and correct as they stand: the `EVENT` object's `startUtc`/`endUtc`/`startIso`/`endIso` and the countdown target, which all bracket the whole event at 17:00–23:00 HKT, not the dinner. `.day-time` is deliberately **not** in the i18n dictionary — the times are numerals and render identically in all three languages, so there is nothing to translate.
...
- Trigger schedule:
  ...
  ['gallery',      'egg-r',  1000],
  ```
- ⚠️ **STALE — noted 2026-09-22.** The 2026-08-13 entry below is history: since the 2026-08-17 reorganisation (see Section Order above) `#qa` is a top-level section again and only `#chambolle` is still nested inside `#more`, so the `qa` trigger fires from ordinary scrolling; the accordion also no longer uses `max-height` (see 🗂 More). Kept as written.
...
## Admin Portal (old CLAUDE.md §4b) — backs CLAUDE.md §5: `admin.html` is a navigation hub with no auth gate; the `.admin-nav` strip on the three tools, hidden in print
...
## Seating Planner (old CLAUDE.md §5) — backs CLAUDE.md §1 (13 × 12 = 156, documented as 15 until 2026-09-03) and §5 (the grid mirrors the guest-facing floor plan's clusters)
...
- **2026-09-03: the admin floor plan's CSS-grid layout was redrawn to mirror the guest-facing SVG, per user request** ("I want the floor plan in seating planner follows the guest-facing seating plan arrangement"). Before this, the two tools' groupings actively disagreed — table 10 rendered in the admin tool's bottom ("Foyer-like") section while the real plan always had it in the Room, and the left/right table numbers were interleaved in an order that matched neither cluster. Now the `.pos-tN` grid-column/grid-row classes and the `TABLES` array's order both follow the same clusters as `wedding-invitation.html`'s floor-plan SVG: table 9 flanks the stage alone (`grid-row: 1`, left column only — nothing sits in the mirrored right-column cell, matching the real plan's deliberately empty upper-right), 10/6 and 3/7 form row 2 (left cluster then right cluster), 8/2 and 1/5 form row 3, then the door divider, then 11/13 and 12/15 as the Foyer's 2×2 in rows 5–6. This is a CSS-grid abstraction (table cards with functional drag-and-drop seat lists), not a pixel trace of the SVG — the goal was matching groupings and left/right/Foyer structure, not exact geometry. Verified via Playwright (auth-gate bypassed, `renderTables()` called directly with an empty guest list): all 13 table cards render in the expected grid cells, stage sits alone at top-center with only table 9 beside it, and the Foyer's four tables form a clean 2×2 below the divider.
...
## WhatsApp Outreach (old CLAUDE.md §5b) — backs CLAUDE.md §4: `rsvp_status` is an expectation not an answer, `attendance_truth` / `attendanceOf()` move together, `[PREVIEW]` rows excluded, later source wins, `.from('guests')` not the view, Attending vs Outreach are two statuses
...
## Roster / RSVP / Guest Management (old CLAUDE.md §6) — backs CLAUDE.md §1 (seat headroom against 156), §2 (the seed snapshot, the Excel tracker), §4 (`guest_number` is the identity and is sparse, regenerate the seed after any hand edit, `admin_*` are views, `guests.phone` write-through, outreach templates in Supabase, `groupOverrides` gap, audit SQL one block at a time) and §5 (Excel tracker rules)

### 6. RSVP / Guest Management

#### Guest List Stats — live roster, verified 2026-08-25

**`schema_seed.sql` carries the live roster: 163 numbered guests.** Regenerated with `supabase/migrations/export_roster_as_seed.sql` from the live database and now fully in step with it — `guests` holds 165 rows (163 + the 2 `[PREVIEW]`), and loading the file reproduces exactly that.

⚠️ **The couple uninvited 34 people after the spreadsheet was made.** The seed's INSERT is an upsert that never deletes, so until it was regenerated a full re-run would have resurrected all 34 and issued them invitation codes. Verified against a real Postgres 16: loading the file gives 165 rows, every row has an invitation code, no duplicate `guest_number`, `attendance_truth` reports 163, and **re-running the whole file a second time still gives 165** — the 34 stay gone. Whenever guests are added or removed by hand, regenerate this block again; that step was skipped once and is what created the hazard.

ℹ️ **Eva Ng is `guest_number` 200**, not 199 — she was added by hand with no number, and the `MAX + 1` statement was run twice. Harmless; numbers only need to be unique.

ℹ️ **Guest numbers are sparse** — 1–200 with 37 gaps, since uninviting removes the row but never renumbers. Do not assume `guest_number` is contiguous or that `MAX` equals the headcount. The export's `WHERE guest_number IS NOT NULL` filter is there to skip the `[PREVIEW]` rows, but it silently skips any hand-added guest with no number too — that is how Eva Ng was nearly lost. Run the sanity checks at the foot of `export_roster_as_seed.sql` after every export.
...
| Field | Count (163 seeded rows) |
|--------|-------|
| Expected Yes | 155 |
| Expected No | 3 |
| ⏳ Pending | 5 |
| 女方 (Christy's side) | 105 |
| 男方 (Mitchell's side) | 58 |
| Kids | 4 |
...
## Pending / TODO (old CLAUDE.md §7) — backs CLAUDE.md §6 open items: no contact route on the site, the ~1 MB file, the pending guests
